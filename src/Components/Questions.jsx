...
const FAQItem = ({ question, answer }) => {
  ...
  return (
    <div
      className="w-full bg-zinc-800 rounded-2xl mb-4 overflow-hidden cursor-pointer"
      onClick={() => setIsOpen(!isOpen)}
    >
      <div className="flex items-center justify-between p-5 group">
        <h1 className="font-semibold text-lg group-hover:text-yellow-400">
          {question}
        </h1>
        <i
          className={`ri-arrow-${
            isOpen ? "up" : "down"
          }-s-line text-lg text-zinc-500`}
        ></i>
      </div>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div
            ...
            className="px-5 overflow-hidden text-sm text-zinc-400"
          >
...
function Questions() {
  ...
  return (
    <div className="w-full xl:min-h-screen h-fit bg-[#1a1a1a] px-6 text-white py-10 relative overflow-hidden">
      ...
      <div className="about text-center mb-10">
        <p className="text-yellow-400 font-semibold mb-2">Common Questions</p>
        <h1 className="font-bold text-4xl mb-4">Frequently Asked Questions</h1>
        <p className="text-lg w-[60%] mx-auto text-zinc-600">
      ...
      <div className="flex flex-col items-center gap-4 w-full">
        <input
          ...
          className="w-[60%] h-12 bg-zinc-800 rounded-full border border-zinc-700 outline-none px-5 text-sm focus:border-zinc-500 focus:shadow focus:shadow-white/50 mb-6"
        />

        <div className="faqs w-[60%]">
          {faqs.map((faq, index) => (
